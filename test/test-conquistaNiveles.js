	...
	request(options,function(error,response,body){
		if (!error && response.statusCode==200){
    		...
    		if (jsonResponse.email!=""){
	    		...
	    		actualizarUsuario(jsonResponse._id,jsonResponse.email,jsonResponse.password);
	    		//eliminarUsuario(jsonResponse._id);
	    	}
	    	else{
...
function actualizarUsuario(id,email,password){
	...
	console.log("---------------------------------------------");
	console.log("3. El usuario: "+email+" actualiza el nombre");
	console.log("---------------------------------------------");
	...
	request(options,function(error,response,body){
		if (!error && response.statusCode==200){
    		...
    		//nivelCompletado(jsonResponse._id,20);
    		if (jsonResponse.email!=""){
	    		console.log("Usuario "+jsonResponse.email+" ha actualizado su nombre \n");
	    		eliminarUsuario(jsonResponse._id);
	    	}
	    	else{
	    		console.log("El usuario "+email+" NO pudo iniciar la sesión \n");
